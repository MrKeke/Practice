...
export const Product = ({ id, price, name, ingridients, img }) => {
	...
	const idFinder = async (id, e) => {
      ...
      e.target.parentNode.parentNode.parentNode.parentNode.firstChild
		...
		const idChecker = await axios
			.get(selectedItems)
			.then((el) => el.data.map((el) => el.id));
		await axios.get(`http://localhost:3000/${validName}`).then((e) =>
			e.data.map((el) => {
	...
	return (
		<div
			id={id}
			className="relative p-3 box-border bg-white text-base rounded flex flex-col gap-6 hover:cursor-pointer overflow-hidden"
		>
			<div>
				<div className="h-64">
					<img
			...
			<div className="absolute flex text-end bottom-3 right-3 flex-auto">
				<div className="text-[#FF7010] pt-3 pr-4 text-left font-semibold ">
          от {price} ₽
				</div>
				<button
					onClick={(e) => idFinder(id, e)}
					className="bg-[#FF7010] px-5 py-1 text-white text-base rounded transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 duration-300"
				>
          Выбрать
				</button>
			</div>
